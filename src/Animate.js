import React from "react";
import "animate.css";
import "./infinite.css";
import "./App.css";
// import "@fortawesome/fontawesome-svg-core";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faPlay,
  faPause,
  faBackward,
  faForward,
} from "@fortawesome/free-solid-svg-icons";

function Animate() {
  return (
    <div>
      <header>
        <h1 className={"test-brand animate__animated animate__slideInRight"}>
          <p className={"animate__animated animate__pulse"}>Play.All()</p>
        </h1>
        <h1 className={"test-header animate__animated animate__slideInLeft"}>
          I am sliding in from the left.
        </h1>
        <div className={"music-player-container"}>
          <div className={"music-player"}>
            <img src="https://i.scdn.co/image/ab67616d00001e029e1cfc756886ac782e363d79"></img>
            <div className={"track-details"}>
              <p>The Less I Know The Better</p>
              <p>Tame Imapala</p>
            </div>
            <FontAwesomeIcon icon={faBackward} />
            <FontAwesomeIcon icon={faPlay} />
            <FontAwesomeIcon icon={faPause} />
            <FontAwesomeIcon icon={faForward} />
          </div>
        </div>
        <div className={"test-room-container"}>
          <div className={"test-room-text"}>
            <h2>Active Rooms</h2>
            <p>Click a room to join and listen</p>
          </div>
          <div className={"test-room"}>
            <img src="https://i.scdn.co/image/ab67616d00001e029e1cfc756886ac782e363d79"></img>
          </div>
          <div className={"test-room"}>
            <img src="https://i.scdn.co/image/ab67616d00001e029e1cfc756886ac782e363d79"></img>
          </div>
          <div className={"test-room"}>
            <img src="https://i.scdn.co/image/ab67616d00001e029e1cfc756886ac782e363d79"></img>
          </div>
          <div className={"test-room"}>
            <img src="https://i.scdn.co/image/ab67616d00001e029e1cfc756886ac782e363d79"></img>
          </div>
        </div>
      </header>
    </div>
  );
}

export default Animate;
